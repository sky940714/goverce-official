...
const InputStep = ({ onNext }) => {
  ...
  const handleImageChange = (e) => {
    ...
    reader.onload = () => {
      setImage({ base64: reader.result.split(',')[1], mimeType: file.type, preview: reader.result });
    };
    ...
    <motion.div {...PAGE} className="w-full max-w-xl mx-auto">
      <div className="text-center mb-10">
        ...
        <h1 className="text-4xl md:text-5xl font-black tracking-tighter text-[#1A1A1A] mb-4 leading-tight">
          AI 搜不到你的店？<br /><span className="text-[#FF6B00]">30 秒找出原因</span>
        </h1>
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-8">
        <div className="space-y-4">
          <div>
            ...
            <input value={name} onChange={e => setName(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSubmit()} placeholder="例：老王牛肉麵" className={INPUT_CLS} />
          </div>
          <div>
            <label className="text-xs font-black text-gray-400 tracking-widest uppercase mb-2 block">地區 / 縣市</label>
            <input value={area} onChange={e => setArea(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSubmit()} placeholder="例：台北市大安區" className={INPUT_CLS} />
          </div>
          ...
          <div>
            ...
            {!image ? (
              <button type="button" onClick={() => fileInputRef.current?.click()}
                ...
                <Upload size={22} /><span className="font-bold text-sm">點擊上傳截圖</span><span className="text-xs">JPG / PNG · 最大 5MB</span>
              </button>
            ) : (
              <div className="relative rounded-2xl overflow-hidden border border-gray-200 bg-gray-50">
                ...
                <div className="absolute bottom-2 left-2 flex items-center gap-1.5 bg-black/50 rounded-full px-3 py-1">
                  <ImageIcon size={11} className="text-white" /><span className="text-white text-[11px] font-bold">截圖已上傳</span>
                </div>
              </div>
            )}
  ...
  <motion.div {...PAGE} className="w-full max-w-sm mx-auto text-center">
    <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-10">
      ...
      <div className="space-y-3">
        <button onClick={onConfirm} className="w-full bg-[#FF6B00] text-white py-4 rounded-2xl font-black text-base hover:bg-[#e85f00] transition-all active:scale-[0.98] shadow-md shadow-[#FF6B00]/20">對，就是這家！</button>
        <button onClick={onBack} className="w-full text-gray-400 py-3 font-bold hover:text-[#1A1A1A] transition-all text-sm">不對，重新輸入</button>
      </div>
    </div>
  </motion.div>
);
  ...
  useEffect(() => {
    const animPromise = new Promise(resolve => {
      ...
      const advance = () => {
        ...
        setCurrentIdx(idx);
        setTimeout(() => { setDone(prev => [...prev, idx]); idx++; advance(); }, PROCESS_STEPS[idx].ms);
      };
      advance();
    });

    const body = {
      name: business.name, area: business.area,
      ...(business.image && { imageBase64: business.image.base64, imageMimeType: business.image.mimeType }),
    };

    const apiPromise = fetch('/api/audit/run', {
      method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body),
    }).then(async r => {
      if (!r.ok) { const d = await r.json().catch(() => ({})); throw new Error(d.error || '伺服器錯誤'); }
      return r.json();
    });
  ...
  return (
    <motion.div {...PAGE} className="w-full max-w-lg mx-auto">
      <div className="text-center mb-10">
        ...
        <p className="text-gray-500 font-medium">{business.image ? '已附上截圖，分析更精準 · 約需 30 秒' : '請稍候，約需 30 秒'}</p>
      </div>
      {apiError && <div className="bg-red-50 border border-red-200 rounded-2xl p-4 mb-6 text-red-600 font-bold text-sm text-center">{apiError}</div>}
        ...
        {PROCESS_STEPS.map((step, i) => {
          ...
          return (
            <div key={step.id} className="flex items-center gap-5">
              ...
              <span className={`font-bold text-base transition-colors ${isCompleted ? 'text-green-500 line-through decoration-green-300' : isCurrent ? 'text-[#1A1A1A]' : 'text-gray-400'}`}>
                {step.label}
              </span>
            </div>
          );
    ...
    <motion.div {...PAGE} className="w-full max-w-2xl mx-auto">
      <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-8 md:p-10 mb-4">
        <div className="flex flex-col md:flex-row items-center gap-8">
          <div className={`w-36 h-36 rounded-full border-4 ${ts.border} ${ts.bg} flex flex-col items-center justify-center flex-shrink-0`}>
          ...
          <div className="flex-1 text-center md:text-left">
            ...
            <p className="text-gray-500 font-medium leading-relaxed">
              {result.total < 40 ? '你的店在 AI 搜尋中幾乎是隱形的。當消費者問 ChatGPT「推薦附近的餐廳」，你的競爭對手出現了，但你沒有。' : '你的店在部分 AI 平台可被找到，但仍有明顯優化空間。'}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-8 space-y-5">
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-2">
            {TIMES.map(t => (
              <button key={t.id} onClick={() => setForm({ ...form, time: t.id })}
                className={`py-3 px-2 rounded-xl font-black text-xs transition-all ${form.time === t.id ? 'bg-[#FF6B00] text-white shadow-md shadow-[#FF6B00]/20' : 'bg-gray-50 border border-gray-200 text-gray-500 hover:text-[#1A1A1A] hover:border-gray-300'}`}>
                {t.label}
              </button>
            ))}
          </div>
...
const ReportStep = ({ result, onRestart }) => (
  <motion.div {...PAGE} className="w-full max-w-2xl mx-auto">
        ...
        return (
          <div key={cat.label} className={`rounded-2xl p-6 border ${s.bg} ${s.border}`}>
            <div className="flex items-center justify-between mb-3">
              ...
              <span className={`font-black text-xl ${s.color}`}>{cat.score}<span className="text-gray-400 text-sm font-bold">/{cat.max}</span></span>
            </div>
      ...
      {Object.entries(result.actions).map(([key, items]) => {
        ...
        return (
          <div key={key} className={`rounded-2xl p-6 border ${cls}`}>
            <div className="flex items-center gap-2 mb-4"><Icon size={14} /><span className="font-black text-xs tracking-widest uppercase">{label}</span></div>
            <ul className="space-y-2.5">
              {items.map((item, i) => (
                <li key={i} className="flex items-start gap-3 text-gray-700 text-sm font-medium">
                  <span className="mt-0.5 w-5 h-5 rounded-full border border-current flex-shrink-0 flex items-center justify-center text-[10px] font-black">{i + 1}</span>
                  {item}
                </li>
              ))}
            </ul>
    ...
    <div className="bg-[#FF6B00]/10 border border-[#FF6B00]/20 rounded-2xl p-6 text-center mb-5">
      ...
      <p className="text-gray-600 font-medium text-sm leading-relaxed">GOVERCE GEO 顧問會在你選定的時段致電，為你量身規劃完整改善方案。</p>
    </div>
  ...
  return (
    <div className="min-h-screen bg-[#F5F5F5] pt-32 pb-24 px-5">
      <AnimatePresence mode="wait">
        {step === 'input' && <InputStep key="input" onNext={b => { setBusiness(b); go('confirm'); }} />}
        {step === 'confirm' && <ConfirmStep key="confirm" business={business} onConfirm={() => go('processing')} onBack={() => go('input')} />}
        {step === 'processing' && <ProcessingStep key="processing" business={business} onComplete={r => { setResult(r); go('summary'); }} />}
        {step === 'summary' && <SummaryStep key="summary" result={result} onUnlock={() => go('leadform')} />}
        {step === 'leadform' && <LeadFormStep key="leadform" slug={result?.slug} onSubmit={() => go('report')} />}
        {step === 'report' && <ReportStep key="report" result={result} onRestart={() => { setBusiness(null); setResult(null); go('input'); }} />}
      </AnimatePresence>
    </div>
  );
};
